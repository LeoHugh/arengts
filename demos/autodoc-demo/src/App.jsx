...
const callGLM = async (prompt, systemInstruction = "") => {
  try {
    const messages = [];
    ...
    console.log("GLM Request:", JSON.stringify({ model: "glm-4.7", messages }, null, 2));
    ...
    const data = await response.json();
    console.log("GLM Response data:", data);

    return data.choices?.[0]?.message?.content || "Error: No content in response.";
  } catch (e) {
    console.error("GLM Request Failed:", e);
    return "Error: Could not connect to GLM.";
  }
};
...
export default function SelfEvolutionDemo() {
  ...
  const runAgent = async () => {
    ...
    // 2. SEND TO GLM
    addLog(`Sending content to GLM for processing...`, 'info');
    ...
    const generatedText = await callGLM(userPrompt, fullSystemPrompt);

    setIsGeminiThinking(false);

    if (generatedText) {
      addLog("GLM generation complete.", 'success');
  ...
  const handleCommit = async () => {
    ...
    const extractedRule = await callGLM(analysisPrompt, "You are a helpful analyst.");
    setIsGeminiThinking(false);
      ...
      <header className="flex items-center justify-between px-6 py-4 bg-slate-900 border-b border-slate-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">Powered by GLM ✨</p>
          </div>
        </div>
              ...
              <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center z-10">
                <div className="bg-slate-900 border border-slate-700 p-8 rounded-xl shadow-2xl max-w-md w-full">
                  <div className="flex flex-col items-center text-center space-y-4">
                    ...
                    <h2 className="text-xl font-bold text-white">GLM is Thinking...</h2>
            ...
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
               <div className="text-center space-y-4 max-w-lg">
                 <div className="p-4 bg-slate-900/80 border border-slate-800 rounded-lg text-slate-400 text-sm leading-relaxed backdrop-blur-md">
                   <p className="font-semibold text-slate-200 mb-2 flex items-center justify-center gap-2"><Sparkles className="w-4 h-4 text-purple-400"/> GLM-Powered Shadow Mode</p>
                   ...
                   <p>3. Commit to let GLM <strong>analyze your style</strong> and update its rules.</p>
                 </div>
               </div>
            </div>
          )}
